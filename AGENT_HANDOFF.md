...
> 最後更新：2026-07-02 · **commit `c3252df`**（Render Starter 遷移完成）· BUG-2026-001 **resolved**
...
**Local、GitHub `main`、Render 三邊 `version` 必須相同**才算部署完成。每次 push 後 CI 會觸發 Deploy Hook；若落後可手動 `bash deploy/render-sync.sh`。
...
curl -s https://oikonomia.onrender.com/api/version | python3 -m json.tool
# 預期：render: true, data_dir: "/data", db_path: "/data/oikonomia.db"
```

**Render 架構約束**：生產用 gunicorn（`render.yaml`）；持久資料只在 `/data`（DB、uploads、`.secret_key`、`.gm_pin`、`.combat_v2`）；`preDeployCommand` 跑 `deploy/render-predeploy.sh` 寫 `.deploy-version`。Deploy Hook URL 存 GitHub Secret `RENDER_DEPLOY_HOOK`，**勿 commit**。
...
- Render: curl -s https://oikonomia.onrender.com/api/version | python3 -m json.tool
  確認 version 與 local 相同；render: true；data_dir: /data；markers 含 combat_v2

Render 若落後：push main（CI 觸發 hook）或 bash deploy/render-sync.sh，你再 curl 確認。
